...
- `/admin_hedra_voice_assets [поиск]` - проверить `GET /assets?type=voice`.
- `/admin_find_voice <name>` - найти голос в `/voices` и `/assets?type=voice`.
- `/admin_import_voice_name <name>` - импортировать голос по имени из `/voices` или `/assets?type=voice`.
